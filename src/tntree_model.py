...
class BinaryTensorTree(nn.Module):
    ...
    def forward_hierarchical(self, inputs, use_log_scale=False):
        """
        Forward pass using hierarchical computation (works for any dimension).
        
        Args:
            inputs: Per-leaf inputs or [B, F] matrix
            use_log_scale: If True, use log-scale stabilization to prevent numerical collapse
                          (Default: False - not needed with polynomial features)
        """
        if isinstance(inputs, torch.Tensor):
        ...
        node_vals = {}
        log_scales = {}  # Track log-scale factors for stability
        
        # leaves
        for lid, x in zip(self.leaf_ids, xs):
            core = self.cores[str(lid)]
            if batched:
                val = torch.tensordot(x, core, dims=([1], [0]))
            else:
                val = torch.tensordot(x, core, dims=([0], [0]))
            
            if use_log_scale:
                # Normalize by max abs value to keep tensors O(1)
                if batched:
                    scale = val.abs().amax(dim=-1, keepdim=True) + 1e-8
                else:
                    scale = val.abs().max() + 1e-8
                node_vals[lid] = val / scale
                log_scales[lid] = torch.log(scale)
            else:
                node_vals[lid] = val
                log_scales[lid] = 0.0
        ...
        for n in reversed(self._preorder(self.root)):
            ...
            if n == self.root:
                # Root: compute final output and apply accumulated scales
                if self.out_dim == 1:
                    if batched:
                    ...
                    if batched:
                        tmp = torch.tensordot(left, core, dims=([1], [0]))
                        y = (right.unsqueeze(-1) * tmp).sum(dim=1)
                    else:
                        tmp = torch.tensordot(left, core, dims=([0], [0]))
                        y = (right.unsqueeze(-1) * tmp).sum(dim=0)
                
                # Apply accumulated log-scales
                if use_log_scale:
                    total_log_scale = log_scales[L] + log_scales[R]
                    y = y * torch.exp(total_log_scale)
                return y
            else:
                # Internal node: contract and normalize
                if batched:
                    tmp = torch.tensordot(left, core, dims=([1], [0]))
                ...
                if use_log_scale:
                    # Normalize by max abs to keep O(1) and accumulate scales
                    if batched:
                        scale = up.abs().amax(dim=-1, keepdim=True) + 1e-8
                    else:
                        scale = up.abs().max() + 1e-8
                    node_vals[n] = up / scale
                    log_scales[n] = log_scales[L] + log_scales[R] + torch.log(scale)
                else:
                    node_vals[n] = up
                    log_scales[n] = 0.0
    ...
    def forward(self, inputs):
        ...
        return self.forward_hierarchical(inputs, use_log_scale=self.use_log_scale)
...
def make_balanced_binary_tensor_tree(
    ...
    Args:
        ...
        use_log_scale: Whether to use log-scale stabilization in forward pass
        seed: Random seed
        device: Device to place tensors on
        dtype: Data type for tensors
