...
class PolynomialFeatureMap(nn.Module):
    """
    Fixed polynomial feature map: ψ(x) = [x², x] (no bias here, added later).
    
    This is particularly effective for polynomial targets and ensures ψ(0) = 0.
    No learnable parameters - pure mathematical transformation.
    
    Input:  x ∈ R^{B×D}
    Output: z ∈ R^{B×D×2}  where z[..., 0] = x², z[..., 1] = x
    """
    def __init__(self):
        super().__init__()
        self.out_dim = 2  # [x², x]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Forward pass: x: [B, D] -> [B, D, 2] where [..., 0]=x², [..., 1]=x
        """
        if x.ndim != 2:
            raise ValueError("PolynomialFeatureMap expects input of shape [B, D]")
        B, D = x.shape
        x2 = x.pow(2)
        z = torch.stack([x2, x], dim=-1)  # [B, D, 2]
        return z
...
class FeatureMappedTN(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            use_polynomial_features: If True, use fixed [x², x] features instead of learned MLP
        ...
        # Create feature map: polynomial or learned MLP
        if use_polynomial_features:
            self.feature_map = PolynomialFeatureMap()
        else:
...
def make_feature_mapped_tn(
    ...
    Args:
        d_in: Number of input features
        fmap_out_dim: Number of feature map output channels (ignored if use_polynomial_features=True)
        ranks: Tensor network ranks
        out_dim: Output dimension
        fmap_hidden: Hidden layer size in feature map (ignored if use_polynomial_features=True)
        fmap_act: Activation function for feature map (ignored if use_polynomial_features=True)
        use_polynomial_features: If True, use fixed [x², x] features instead of learned MLP
        selector_mode: Selector mode for ablation studies
        seed: Random seed
    ...
    # If using polynomial features, override fmap_out_dim
    if use_polynomial_features:
        actual_fmap_out_dim = 2  # [x², x]
    else:
        actual_fmap_out_dim = fmap_out_dim
    
    leaf_phys_dims = [actual_fmap_out_dim + 1] * d_in  # +1 for bias
    ...
    return FeatureMappedTN(
        tn=tn,
        d_in=d_in,
        fmap_out_dim=actual_fmap_out_dim,
