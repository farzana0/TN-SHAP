...
End-to-end:
  - load dataset + train teacher
  - build ONE shared-grid FewEval masked set
  - train plain BinaryTensorTree on masked rows (retry if train R^2 < threshold)
  - evaluate TN selector vs exact teacher for k=1,2,3
  - save per-idx CSVs, per-run summary, and manifest

Outputs go to:
  <outdir>/<dataset>_seed<seed>_K<K>/baseline_no_fmap/
"""
...
# ----------------- training TN (no feature map) -----------------
def train_plain_tn_on_masked(masked_X: np.ndarray, y_teacher: np.ndarray, ranks: int, seed: int,
                             lr=2e-2, max_epochs=50, tol=1e-7, amp=True, batch_size=None,
                             print_every=10, patience=50):
    set_all_seeds(seed)
    dev = DEVICE
    X_t = torch.tensor(masked_X, device=dev)      # [N,D]
    y_t = torch.tensor(y_teacher, device=dev)     # [N]
    d = masked_X.shape[1]

    # Plain TN, leaves see [x_i, 1] because phys_dim=2 and assume_bias_when_matrix=True
    tn = BinaryTensorTree([2]*d, ranks=ranks, out_dim=1,
                          assume_bias_when_matrix=True, seed=seed, device=dev).to(dev)

    n = X_t.shape[0]
    if batch_size is None:
        batch_size = n if n <= 2048 else min(16384, max(512, n // 16))
    opt = torch.optim.AdamW(tn.parameters(), lr=lr, weight_decay=0.0, eps=1e-8)
    ...
    for ep in range(1, max_epochs+1):
        ...
        if batch_size >= n:
            ...
            scaler.unscale_(opt)
            torch.nn.utils.clip_grad_norm_(tn.parameters(), 2.0)
        ...
        if mse_epoch < best_mse - 1e-12:
            best_mse = mse_epoch
            best_state = {k: v.detach().cpu().clone() for k, v in tn.state_dict().items()}
            noimp = 0
        else:
            noimp += 1
        if (ep % print_every == 0) or ep == 1:
            mse_full, r2_full = eval_full()
            print(f"[TN no-fmap] ep {ep:3d} mse={mse_epoch:.3e} best={best_mse:.3e} full={mse_full:.3e} R2={r2_full:.4f}")
    ...
    return tn, dict(final_mse=final_mse, final_r2=final_r2, best_mse=best_mse, epochs=ep, train_time_s=elapsed)
...
def main():
    ...
    # retry policy
    ap.add_argument("--train-r2-min", type=float, default=0.7)
    ap.add_argument("--train-retries", type=int, default=3)
    args = ap.parse_args()
    ...
    run_dir = os.path.join(base_root, "baseline_no_fmap")
    ensure_dir(run_dir)
    ...
    # 6) train plain TN with retries
    used_seed = int(args.seed)
    used_info = None
    tn = None
    for attempt in range(0, int(args.train_retries) + 1):
        if attempt > 0:
            used_seed = int(args.seed + 100 * attempt)
            print(f"[RETRY no-fmap] attempt {attempt}/{args.train_retries} seed={used_seed}")
        tn, info = train_plain_tn_on_masked(
            masked_X, y_teacher, ranks=args.rank, seed=used_seed,
            lr=2e-2, max_epochs=50, tol=1e-7, amp=True,
            batch_size=None, print_every=10, patience=50
        )
        used_info = info
        if info["final_r2"] >= args.train_r2_min:
            print(f"[OK no-fmap] seed={used_seed} train R^2={info['final_r2']:.4f} >= {args.train_r2_min}")
            break
        else:
            print(f"[LOW no-fmap] seed={used_seed} train R^2={info['final_r2']:.4f} < {args.train_r2_min}")

    torch.save(tn.state_dict(), os.path.join(run_dir, "tn.pt"))

    manifest = dict(
        dataset=args.dataset, seed=int(args.seed), tn_seed=used_seed,
        kpoints=int(K), rank=int(args.rank), grid_nodes=int(mgrid), dim=int(d),
        masked_rows=int(masked_X.shape[0]), device=str(DEVICE),
        tn_info=used_info, fmap_mode="none_plain_[x_i,1]",
