...
Now supports feature-map output dimension ablation (m_fmap) and selector modes.
...
Saves:
  ...
  - t_nodes_shared.npy
  - teacher.pt
  - tn.pt                  # FeatureMappedTN state_dict
  - manifest.json
"""
...
from src.feature_mapped_tn import FeatureMappedTN, make_feature_mapped_tn
...
# ----- TN student (updated to support fmap_out_dim + selector_mode) -----
def train_tn_on_masked(masked_X: np.ndarray, y_teacher: np.ndarray,
                       ranks: int, seed: int,
                       max_epochs=100, tol=1e-6, lr=2e-3, amp=True, batch_size=None,
                       fmap_hidden: int = 32, fmap_out_dim: int = 1, selector_mode: str = "none",
                       print_every: int = 50, patience: int = 200, target_r2: Optional[float] = None,
    ...
    # Build wrapped TN with per-leaf phys_dim = fmap_out_dim + 1 (bias)
    ...
    for ep in range(1, max_epochs+1):
        ...
        if need_log:
            ...
            print(f"[TN] ep {ep:4d}  mse_epoch={mse_epoch:.4e}  best_mse={best_mse:.4e}  full_mse={mse_full:.4e}  R2={r2_full:.4f}{' *' if improved else ''}")
...
# ----- Main + tests/ablation -----
def run_smoke_test_fmap_dims():
    print("\n[TEST] Smoke test: higher-dim feature maps")
    B, F = 5, 4
    x = torch.randn(B, F, device=DEVICE)
    for m in [1, 2, 4]:
        model = make_feature_mapped_tn(
            d_in=F, fmap_out_dim=m, ranks=8, out_dim=1,
            fmap_hidden=16, fmap_act="relu",
            selector_mode="per_channel", seed=0, device=DEVICE
        )
        y0 = model(x)  # no selectors
        sel = torch.ones(1, F, m, device=DEVICE) * 0.75
        y1 = model(x, selectors=sel)
        assert y0.shape == y1.shape, "Output shape mismatch"
        print(f"  m={m}: ok  (y.shape={tuple(y0.shape)})")
    print("[TEST] Passed.\n")

def run_ablation_diabetes(outdir_root: str, seed: int, kpoints: int, ranks: int, m_list: list):
    print("\n[ABL] Diabetes ablation over fmap_out_dim =", m_list)
    ensure_dir(outdir_root)
    ds_name, Xtr, ytr, Xva, yva, Xte, yte, sx, sy = load_dataset_by_name("diabetes", seed)
    teacher = train_teacher(Xtr, ytr, Xva, yva, seed=seed, lr=3e-3)

    rng = np.random.default_rng(seed)
    n_te = Xte.shape[0]; K = min(kpoints, n_te)
    chosen = rng.choice(n_te, size=K, replace=False)
    Xtargets = Xte[chosen].astype(np.float32)
    D = Xtargets.shape[1]

    # shared grid with m_cheb = D (typical)
    t_nodes = chebyshev_nodes_01(D)
    masked_X, meta_rows = generate_masked_dataset_single_grid(Xtargets, t_nodes, max_pairs=None, max_triplets=None)
    ...
    for m_fmap in m_list:
        print(f"[ABL] Training TN with fmap_out_dim={m_fmap}")
        tn, info = train_tn_on_masked(
            masked_X, y_teacher, ranks=ranks, seed=seed,
            max_epochs=50, tol=1e-7, lr=2e-2, amp=True, batch_size=None,
            fmap_hidden=32, fmap_out_dim=m_fmap, selector_mode="none",
            print_every=10, patience=50, target_r2=None, compile_model=False
        )
        rows.append({"fmap_out_dim": m_fmap, **info})

    import pandas as pd
    df = pd.DataFrame(rows)
    out_csv = os.path.join(outdir_root, f"diabetes_ablation_fmap_sizes_seed{seed}.csv")
    df.to_csv(out_csv, index=False)
    print(f"[ABL] Saved: {out_csv}\n")
...
def main():
    ...
    ap.add_argument("--m", type=int, default=None, help="# Chebyshev nodes for path (shared). Default: n (features).")
    ...
    ap.add_argument("--tn-fmap-out-dim", type=int, default=1, help="Output channels m of feature map (per feature)")
    ap.add_argument("--tn-selector-mode", type=str, default="none", choices=["none","per_feature_scalar","per_channel"])
    ap.add_argument("--tn-print-every", type=int, default=50)
    ap.add_argument("--tn-patience", type=int, default=150)
    ap.add_argument("--tn-target-r2", type=float, default=None)
    ap.add_argument("--tn-compile", action="store_true")
    # Tests / ablation:
    ap.add_argument("--run-tests", action="store_true", help="Run quick smoke tests for m>1 maps and exit")
    ap.add_argument("--ablate-fmap-sizes", type=str, default="", help="Comma-separated list, e.g., 1,2,4,8; runs a Diabetes ablation and exits")
    args = ap.parse_args()

    if args.run_tests:
        run_smoke_test_fmap_dims()
        return

    if args.ablate_fmap_sizes:
        m_list = [int(s) for s in args.ablate_fmap_sizes.split(",") if s.strip()]
        outroot = os.path.join(args.outdir, "ablations")
        run_ablation_diabetes(outroot, args.seed, args.kpoints, args.rank, m_list)
        return
    ...
    tn, tn_info = train_tn_on_masked(
        ...
        fmap_out_dim=args.tn_fmap_out_dim,
        selector_mode=args.tn_selector_mode,
        print_every=args.tn_print_every,
        patience=args.tn_patiance if hasattr(args, "tn_patiance") else args.tn_patience,  # typo-safe
        target_r2=args.tn_target_r2,
        compile_model=args.tn_compile
    )

    # 7) Save
    run_root = os.path.join(args.outdir, f"{args.dataset}_seed{args.seed}_K{K}_mgrid{m_shared}_mfmap{args.tn_fmap_out_dim}")
    ensure_dir(run_root)
    ...
    manifest = dict(
        ...
        tn_info=tn_info,
        fmap_out_dim=int(args.tn_fmap_out_dim),
        selector_mode=str(args.tn_selector_mode)
    )
