...
"""
Multilinear-only Shapley experiments (orders 1–8) using your BinaryTensorTree.

Experiments:
  1) GT = low-rank TN (easy)         -> student TN (same rank)
  2) GT = higher-rank TN (harder)    -> student TN (under-ranked)
  3) GT = generic multilinear (<=3)  -> student TN

Shapley interactions (SII) are computed at a single anchor x0 via:
  - GT: exact enumeration of v(S) (either TN-GT or generic coeffs)
  - Student TN: selector-based batched coalition eval v_hat(S)

Outputs:
  - Per experiment: models, anchor x0, SII_{k} (k=1..8) JSONs for GT & student, summary with per-order R².
"""
...
def make_gt_tn(d: int, rank: int, seed: int = 11):
    ...
    tn = make_balanced_binary_tensor_tree(
        ...
        ranks=rank,
        out_dim=1,
        assume_bias_when_matrix=True,  # convenient for matrix input
        seed=seed,
...
def shapley_all_orders_upto_d(v_cache: Dict[int, float], d: int) -> Dict[int, Dict[Tuple[int, ...], float]]:
    """
    Compute SII for all orders 1..d.
    """
...
def r2_per_order_all(gt: Dict[int, Dict[Tuple[int, ...], float]],
                     est: Dict[int, Dict[Tuple[int, ...], float]],
                     atol: float = 1e-8,
                     rtol: float = 1e-5) -> Dict[str, float]:
    """
    R² per order. For singleton cases (e.g., order=d), return 1.0 if equal within tol, else 0.0.
    """
    out = {}
    max_k = max(gt.keys())
    for k in range(1, max_k + 1):
        ...
        if len(y) < 2:
            out[f"order_{k}"] = float(1.0 if np.isclose(y[0], yhat[0], atol=atol, rtol=rtol) else 0.0)
        else:
...
def run_exp_gt_tn(
    ...
    """
    GT is a tensor-tree (rank_gt). Train student TN (rank_student). Compute SII up to order d.
    """
    print(f"\n=== EXP: {name} ===")
    Path(outdir).mkdir(parents=True, exist_ok=True)
    set_seed(seed)

    # Build GT TN (random init defines a multilinear function)
    ...
    # Shapley interactions up to order d
    sii_gt = shapley_all_orders_upto_d(v_gt, d)
    sii_st = shapley_all_orders_upto_d(v_st, d)

    r2s = r2_per_order_all(sii_gt, sii_st)
...
def run_exp_gt_generic(
    ...
    Train student TN (rank_student). Compute SII up to order d (4..8 will be ~0 on GT).
    """
    print(f"\n=== EXP: {name} ===")
    Path(outdir).mkdir(parents=True, exist_ok=True)
    set_seed(seed)
...
def main():
    ...
    # 3) Generic multilinear (<=3). Orders 4..8 GT SII ~ 0.
    run_exp_gt_generic(
